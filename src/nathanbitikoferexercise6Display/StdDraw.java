...
public class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    public static double currentScale = 0;
    public static Maze maze;
    private static int mazeXOffset = 0;
    private static int mazeYOffset = 0;
    // pre-defined colors
    ...
    public static void init() {
        ...
        setXscale(-10,width+10);
        setYscale(height+10,-10);
...
//        offscreen.addRenderingHints(hints);
        ...
        {
            draw.add(frame.getContentPane().getComponent(i));
        }
    ...
    *************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        ...
        int col = (int)((mouseY())/(2.0*currentScale));
        int row = (int)((mouseX()-300)/(2.0*currentScale));
        System.out.println("Row: " + row + "Col: "+ col);
        if (maze != null)
            if (row > 0 && col > 0 && row < maze.cellArray.length && col < maze.cellArray[0].length)
                if (maze.cellArray[row][col] != maze.WALL)
                {
                    if (e.getButton() == 1)
                    {    
                        maze.startCol = col;
    ...
    *************************************************************************/
    ...
    static void showHistogram() {
        clear();
        setPenColor(BLACK);
        double[] data = new double[maze.timeList.size()];
        for (int i = 0; i < maze.timeList.size(); i++)
        {
            data[i] = maze.timeList.get(i);
        }
        Histogram myHistogram = new Histogram((int) StdStats.max(data) + 1);
        for (int i = 0; i < data.length; i++)
            myHistogram.addDataPoint((int)data[i]);
        myHistogram.draw();
        setXscale(-10,width+10);
        setYscale(height+10,-10);

        StdDraw.textLeft(10, 500, "Min time: " + StdStats.min(data));
        StdDraw.textLeft(10, 530, "Ave time: " + StdStats.mean(data));
        StdDraw.textLeft(10, 560, "Max time: " + StdStats.max(data));
    }
    public static void displayMaze(){
        ...
        currentScale = height/(maze.cellArray.length*2.0);
        System.out.println(currentScale);
        //scale = 10;
        mazeXOffset = (int)currentScale + 300;
        mazeYOffset = (int)currentScale;
        
        
        for (int row = 0; row < maze.cellArray.length; row++)
        {
            for (int col = 0; col < maze.cellArray[0].length; col++)
            {
                if (maze.cellArray[row][col] == WALL)
                {
                    StdDraw.filledSquare((row*currentScale*2.0) + mazeXOffset, (col*currentScale*2.0)+ mazeYOffset, currentScale);
                }
            }
        }
        StdDraw.setPenColor(GREEN);
        if (maze.startRow != 0 || maze.startCol !=0)
            StdDraw.filledSquare((maze.startRow*currentScale*2.0) + mazeXOffset, (maze.startCol*currentScale*2.0)+ mazeYOffset, currentScale);
        StdDraw.setPenColor(BLUE);
        if (maze.endRow != 0 || maze.endCol !=0)
            StdDraw.filledSquare((maze.endRow*currentScale*2.0) + mazeXOffset, (maze.endCol*currentScale*2.0)+ mazeYOffset, currentScale);
        StdDraw.setPenColor(RED);
        for (Cell c : maze.path)
        {
            StdDraw.filledSquare((c.row*currentScale*2.0) + mazeXOffset, (c.col*currentScale*2.0)+ mazeYOffset, currentScale);
        }
